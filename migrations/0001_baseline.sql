-- Baseline: the schema as it stood before this directory existed (schema.sql
-- of worker ce12afc). Every statement is IF NOT EXISTS, so applying it to the
-- live database, which already has these tables (calendar_tokens was created
-- on first use by src/handlers/calendar.ts), changes nothing, and leaves its
-- upstream tables and columns alone. Never edit a migration once applied:
-- the next change is the next numbered file, additive.
